...
class Game:
    ...
    def start(self):
        while self.running:
            ...
            self.handle_events()
            self.update()
            self.collision_detection()  # TODO 1 加入碰撞檢測
            self.render()
    ...
    def collision_detection(self):  # TODO 2
        # 小鳥和Pipe群間的碰撞檢測，doKill為False，使用默認的rect檢測
        hit_pipe_list = pygame.sprite.spritecollide(self.bird, self.pipe_group, False)
        # 檢測小鳥和base間的碰撞檢測，使用默認的rect檢測
        hit_base = pygame.sprite.collide_rect(self.bird, self.base)  # 使用self.rect去比較
