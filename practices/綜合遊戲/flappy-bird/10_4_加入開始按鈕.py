...
from FlappyButton import Button  # TODO 0 加入Button 模組
...
class Base(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.image = pygame.transform.scale(self.origin_image, (2 * SCREEN_WIDTH, self.origin_image.get_size()[1]))
...
class Game:
    def __init__(self):
        ...
        self.flappy_bird_text = pygame.image.load('resources/image/flappy_bird_text.png').convert_alpha()
        # 9. 加入buttons
        # TODO 1 初始化Button對象
        self.start_img = pygame.image.load('resources/image/start.png')
        self.start_button = Button(133, 220, self.start_img, 0.35, "start")
        # 變量:
        self.running = True
        self.mark = 0
        self.page = 1
    ...
    def start(self):
        while self.running:
            ...
            self.handle_events()
            if self.page == 1:
                self.beginning_page_update()
                self.beginning_page_render()
            elif self.page == 2:
                self.update()
                self.add_mark()
    ...
    def handle_events(self):
        ...
        # TODO 2 判斷有沒有按下Button對象
        if self.start_button.checkifclick():
            self.page = 2  # 若有，跳轉到頁面2
    ...
    def beginning_page_update(self):
        self.base.update()

    def beginning_page_render(self):
        self.main_surface.blit(self.background_img, (0,0))
        self.main_surface.blit(self.base.image, self.base.rect)
        self.main_surface.blit(self.flappy_bird_text, (110, 150))
        self.start_button.draw(self.main_surface)  # TODO 3 畫Button
        pygame.display.update()
