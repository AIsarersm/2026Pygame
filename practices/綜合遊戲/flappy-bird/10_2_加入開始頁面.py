...
class Game:
    ...
    def start(self):
        while self.running:
            ...
            self.handle_events()
            # TODO 2 在遊戲循環中，通過if else 判斷現在要繪制哪一個頁面
            if self.page == 1:  # TODO 3 page=1時，進行首頁面
                self.beginning_page_render()
            elif self.page == 2:  # page=2時，進行主頁面
                self.update()
                self.add_mark()
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    if self.page == 2:  # TODO 6 需要注意把一些動作放回對應的page中，例如bump只出現在遊戲主頁中
                        self.bird.bump()
    ...
    def beginning_page_render(self):
        # TODO 4 顯示背景、顯示Base、顯示Flappy_bird的文字
        self.main_surface.blit(self.background_img, (0, 0))
        self.main_surface.blit(self.base.image, self.base.rect)
