...
class Pipe(pygame.sprite.Sprite):
    def __init__(self, inverted, init_xpos, show_height):
        ...
        self.is_calculated_mark = False  # TODO 3 定義一個屬性用於判斷有沒有被計分過
    ...
    def set_calculated(self):  # TODO 5 設置已被計分
        self.is_calculated_mark = True

    def get_is_calculated(self):  # TODO 4 獲取是否已被計分
...
class Game:
    ...
    def add_mark(self):
        ...
        if pipe_x < bird_x:
            if not pipe.get_is_calculated():  # TODO 1 要先判斷pipe是否有被計算過
                self.mark += 1
                pipe.set_calculated()  # TODO 2 若計了分，就把pipe設置為「已計分」
                print(self.mark)
