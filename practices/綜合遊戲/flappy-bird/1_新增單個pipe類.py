import pygame
import random

# Game constant
GRAVITY = 0.3
SCREEN_WIDTH = 400
SCREEN_HEIGHT = 600

# Pipe constant
PIPE_WIDTH = 80
PIPE_HEIGHT = 500
TOP_DOWN_PIPE_GAP = 150


class Bird(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.origin_image = pygame.image.load("resources/image/bluebird-downflap.png").convert_alpha()
        self.image = self.origin_image
        self.rect = self.image.get_rect(center=(SCREEN_WIDTH//2, SCREEN_HEIGHT//2))  # 指定坦克的左中點位置
        self.speed = 0

    def update(self):
        self.speed += GRAVITY
        self.image = pygame.transform.rotate(self.origin_image, -3 * self.speed)
        self.rect.y += self.speed
        self.rect = self.image.get_rect(center=(self.rect.centerx, self.rect.centery))

    def bump(self):
        self.speed = -6.5


class Base(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.origin_image = pygame.image.load("resources/image/base.png").convert_alpha()
        self.image = pygame.transform.scale(self.origin_image, (SCREEN_WIDTH, self.origin_image.get_size()[1]))
        self.rect = self.image.get_rect(bottomleft=(0, SCREEN_HEIGHT))  # 指定坦克的左中點位置


# 單個Pipe類  TODO 1
class Pipe(pygame.sprite.Sprite):
    def __init__(self, inverted, init_xpos, show_height):
        super().__init__()
        # 父類初始化
        self.origin_image = pygame.image.load("resources/image/pipe-green.png")
        # scale到希望的長和寬
        self.image = pygame.transform.scale(self.origin_image, (PIPE_WIDTH, PIPE_HEIGHT))

        # 倒放
        if inverted:
            self.image = pygame.transform.flip(self.image, False, True)  # 使用 flip實現上下翻轉
            self.rect = self.image.get_rect()
            self.rect.x = init_xpos
            self.rect.y = show_height - PIPE_HEIGHT  # 為顯示show_height下的rect.y
        else:  # 正放
            self.rect = self.image.get_rect()
            self.rect.x = init_xpos
            self.rect.y = SCREEN_HEIGHT - show_height  # 為顯示show_height下的rect.y

        self.speed = 5

    def update(self):
        self.rect.x -= self.speed
        if self.rect.x < -self.rect.w:
            self.rect.x = SCREEN_WIDTH


class Game:
    def __init__(self):
        pygame.init()
        # 其他對象
        # 0. 定義視窗
        self.main_surface = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
        # 1. 時鐘對象
        self.clock = pygame.time.Clock()
        # 2. 顏色對象
        self.background_color = pygame.color.Color("darkolivegreen2")
        # 3. Bird對象
        self.bird = Bird()
        # 4. Base對象
        self.base = Base()
        # 5. Pipe  TODO 2
        self.pipe = Pipe(False, SCREEN_WIDTH, 300)
        # 變量:
        self.running = True
        # 常量:
        self.FPS = 60

    def start(self):
        while self.running:
            self.clock.tick(self.FPS)
            self.handle_events()
            self.update()
            self.render()

    def handle_events(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                self.running = False
            elif event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    self.bird.bump()

    def update(self):
        # 更新
        self.bird.update()
        self.pipe.update()  # TODO 3

    def render(self):
        # 渲染
        self.main_surface.fill(self.background_color)
        self.main_surface.blit(self.pipe.image, self.pipe.rect)  # TODO 4
        self.main_surface.blit(self.base.image, self.base.rect)
        self.main_surface.blit(self.bird.image, self.bird.rect)
        pygame.display.update()


if __name__ == '__main__':
    game = Game()
    game.start()
